...
async def get_workspace_messages(
    ...
    """Get all messages for a workspace across all threads (chronologically ordered)."""
    try:
        async with get_db_connection() as conn:
            async with conn.cursor(row_factory=dict_row) as cur:
                # Get workspace info
                ...
                # Get all messages across threads
                # Ordered by thread_index (chronological thread order) then pair_index
                # Exclude state_snapshot for efficiency
                if limit:
                    await cur.execute("""
                        SELECT
                            t.thread_id,
                        ...
                        JOIN conversation_query q ON t.thread_id = q.thread_id
                        LEFT JOIN conversation_response r ON q.thread_id = r.thread_id AND q.pair_index = r.pair_index
                        WHERE t.workspace_id = %s
                        ORDER BY t.thread_index ASC, q.pair_index ASC
                        LIMIT %s OFFSET %s
                ...
                return workspace, [dict(row) for row in messages], total_count
